# the main function
Omsda <- function(x, y, lambda = NULL, standardize=NULL, alpha=NULL,nlambda = 100, lambda.factor = ifelse((nobs - nclass) <= 
    nvars, 0.2, 1e-03), dfmax = nobs, pmax = min(dfmax * 
    ...
    ## call Fortran core
    fit <- .Fortran("msda1", obj = double(nlam), nk, nvars, as.double(sigma), as.double(delta), 
