cv.Mmsda <- function(x, y, nfolds = 5, lambda = NULL, lambda.opt = "min", 
    ...) {
    y <- drop(y)
    n <- nrow(x)
    p <- ncol(x)
    ...
    for (i in seq(nfolds)) {
        which <- foldid == i
        # cat("\nFold ", i, " is running.\n")
        fit <- Mmsda(x[!which, , drop = FALSE], y[!which], lambda = lambda, 
            ...)
