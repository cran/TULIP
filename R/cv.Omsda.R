cv.Omsda <- function(x, y, nfolds = 5, lambda = NULL, lambda.opt = "min", 
    ...) {
    y <- drop(y)
    n <- nrow(x)
    p <- ncol(x)
    ...
    for (i in seq(nfolds)) {
        which <- foldid == i
        # cat("\nFold ", i, " is running.\n")
        fit <- Omsda(x[!which, , drop = FALSE], y[!which], lambda = lambda, 
            ...)
    ...
    obj <- list(lambda = lambda, cvm = cvm, cvsd = cvsd, 
        lambda.min = lambda.min, lambda.1se = lambda.1se, model.fit = tmp)
    class(obj) <- "cv.msda.original"
    obj
}