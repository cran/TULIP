# KKT check the output beta matrix
    ...
    for (l in 1:length(lambda)) {
        ...
        for (j in 1:nvars) {
            ...
            if (thetaNorm[j] == 0) {
                ...
                tmp <- dif_norm - los
                if (tmp > 0) 
                  warning("violated at t > 0", tmp, "\n")
            } else {
                tmp3 <- thetaDif[, j] + los * theta[, j]/thetaNorm[j]
                if (any(abs(tmp3) > thr)) 
                  warning("violated at t = 0", tmp3, "\n")
            }
        }
    }
}
...
msda.prep <- function(x, y) {
    ...
    delta=matrix(0, nrow=nvars, ncol=nclass-1)
    for (i in 1:(nclass-1)){
  	delta[,i]=as.matrix(mu[,i+1]-mu[,1])
    }
    delta <- t(delta)
...
Mmsda.prep <- function(x, y) {
  ...
  delta <- t(delta)
  outlist <- list(delta = delta, mu = mu, prior = prior)
  outlist
}
